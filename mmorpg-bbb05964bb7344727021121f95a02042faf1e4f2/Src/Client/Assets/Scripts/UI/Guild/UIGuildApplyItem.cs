...
namespace Assets.Scripts.UI.Guild
{
    class UIGuildApplyItem:ListView.ListViewItem
    {
        ...
        {
            MessageBox.Show(string.Format("要通过[{0}]的公会申请吗?", this.Info.Name), "审批申请", MessageBoxType.Confirm, "通过", "拒绝").OnYes = () =>
              {
        ...
        {
            MessageBox.Show(string.Format("要拒绝[{0}]的公会申请吗?", this.Info.Name), "审批申请", MessageBoxType.Confirm, "通过", "拒绝").OnYes = () =>
            {
