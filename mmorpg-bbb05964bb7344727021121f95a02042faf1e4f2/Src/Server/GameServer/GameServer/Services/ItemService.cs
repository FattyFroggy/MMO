...
namespace GameServer.Services
{
    class ItemService: Singleton<ItemService>
    {
        public ItemService()
        {
            ...
            MessageDistributer<NetConnection<NetSession>>.Instance.Subscribe<SkillBridge.Message.ItemEquipRequest>(this.OnItemEquip);
        }
        ...
        private void OnItemBuy(NetConnection<NetSession> sender, ItemBuyRequest request)
        {
            ...
            Log.InfoFormat("OnItemBuy::character:{0}，Shop:{1},item:{2}", character.Id, request.shopId, request.shopItemId);
        ...
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("OnItemEquip::character:{0}，Slot:{1},Equip:{2}", character.Id, request.Slot, request.isEquip);
