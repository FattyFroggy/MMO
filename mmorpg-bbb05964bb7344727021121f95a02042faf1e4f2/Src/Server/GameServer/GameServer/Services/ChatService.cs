...
namespace GameServer.Services
{
    class ChatService:Singleton<ChatService>
    {
        public ChatService()
        {
            MessageDistributer<NetConnection<NetSession>>.Instance.Subscribe<SkillBridge.Message.ChatRequest>(this.OnChat);
        }
        public void Init()
        {
        ...
        void OnChat(NetConnection<NetSession> sender, ChatRequest request)
        {
            ...
            if (request.Message.Channel == ChatChannel.Private)
            {
                ...
                if (chatTo == null)
                {
                    ...
                    sender.Session.Response.Chat.Erromsg = "对方不在线";
                ...
                else
                {
                    ...
                    request.Message.FromName = character.Data.Name;
